...
def split_sentences(translation, use_char_based_end=True):
    ...
    for item in translation:
        ...
        total_chars = len(translation_text)
        ...
        for i, sentence in enumerate(sentences):
            ...
            if use_char_based_end:
                # Calculate sentence duration based on its proportion of total chars
                sentence_duration = (sentence_chars / total_chars) * (end - start)
                accumulated_duration += sentence_duration
                sentence_end = start + accumulated_duration
            else:
                # Use the original end time for all sentences
                sentence_end = end

            # Ensure the last sentence ends exactly at the original end time
        ...
        fixed_message = [
            {
                'role': 'system',
                'content': (
                    f'You are a professional translator with expertise in translating content across various fields. '
                    f'The current task involves translating the transcript of a video titled "{summary["title"]}". '
                    f'The video summary is: {summary["summary"]}. Translate the provided text into {target_language} accurately, maintaining the original meaning, tone, and style. '
                    f'Ensure the translation is fluent, natural, and uses vocabulary and expressions commonly used by native speakers.'
                )
            },
            {'role': 'user', 'content': 'Translate the following text: "Original Text"'},
            {'role': 'assistant', 'content': 'Translated Text'},
            {'role': 'user', 'content': 'Translate the following text: "Another Original Text"'},
            {'role': 'assistant', 'content': 'Another Translated Text'}
        ]
